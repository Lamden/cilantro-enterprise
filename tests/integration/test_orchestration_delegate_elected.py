...
class TestFullFlowWithMocks(TestCase):
    ...
    def test_vote_new_masternode_elected_and_vote_out(self):
        network = mocks.MockNetwork(num_of_masternodes=2, num_of_delegates=2, ctx=self.ctx)
        ...
        self.loop.run_until_complete(test())
        self.assertFalse(
            candidate.verifying_key in [node.wallet.verifying_key for node in network.masternodes])


    def test_new_masternode_elected_not_prevent_tx(self):
        network = mocks.MockNetwork(num_of_masternodes=2, num_of_delegates=2, ctx=self.ctx)
    ...
    def test_delegate_join_and_participate_in_consensus(self):
        ...
        # self.assertEqual(
        #     2,
        #     len(network.delegates)  # , candidate.verifying_key
        # )
        # network.flush()
